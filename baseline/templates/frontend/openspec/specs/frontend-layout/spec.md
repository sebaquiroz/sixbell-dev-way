...
Define the default repository structure and separation of concerns for a Sixbell frontend project.

## Requirements

### Requirement: Application code and public assets stay explicitly separated

The repository SHALL keep application code under `src/` and public browser-delivered assets under `public/`.

#### Scenario: Team adds new frontend assets or code

- **WHEN** engineers add new application code or public assets to the repository
- **THEN** code SHALL be placed under `src/`
- **AND** browser-delivered static assets SHALL be placed under `public/`
...
The repository SHALL carry its runtime Kiro hooks under `.kiro/hooks/`, including the governance hooks required by The Sixbell-Dev Way and the frontend automation hooks that match the template's scripts.
