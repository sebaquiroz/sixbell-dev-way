...
Define how a repository created from the Sixbell frontend template becomes ready for official OpenSpec workflows in Kiro and GitHub Copilot.
...
A repository created from this template SHALL include `.kiro/hooks/` with the governance hooks required by The Sixbell-Dev Way and the automation hooks compatible with the frontend template.

#### Scenario: New repository is created from the template

- **WHEN** the template is copied into a new repository
- **THEN** `.kiro/hooks/` SHALL contain the governance hooks required by the baseline
- **AND** any hook that executes repository commands SHALL match scripts or files shipped with the frontend template
